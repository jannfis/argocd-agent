...
// Informer is a filtering and customizable SharedIndexInformer for Argo CD
...
func NewAppInformer(ctx context.Context, client appclientset.Interface, namespace string, opts ...InformerOption) *Informer {
	o := &InformerOptions{
	...
	i := &Informer{options: o, appClient: client}
...
func (i *Informer) shouldProcessApp(app *v1alpha1.Application) bool {
...
func (i *Informer) NewAppCallback() NewAppCallback {
	i.lock.RLock()
...
func (i *Informer) SetNewAppCallback(cb NewAppCallback) {
	i.lock.Lock()
	defer i.lock.Unlock()
	i.options.newCb = cb
}
...
func (i *Informer) UpdateAppCallback() UpdateAppCallback {
	i.lock.RLock()
	defer i.lock.RUnlock()
...
func (i *Informer) SetUpdateAppCallback(cb UpdateAppCallback) {
	i.lock.Lock()
	defer i.lock.Unlock()
...
func (i *Informer) DeleteAppCallback() DeleteAppCallback {
	i.lock.RLock()
	defer i.lock.RUnlock()
...
func (i *Informer) SetDeleteAppCallback(cb DeleteAppCallback) {
	i.lock.Lock()
	defer i.lock.Unlock()
...
func (i *Informer) EnsureSynced(d time.Duration) error {
